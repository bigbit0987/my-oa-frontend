...
### 3. 企业级鲁棒性设计 (Robustness & Developer Experience)
*   **样式同步 (Token Bridge)**: 
    *   *决策*: 禁止在 Tailwind 中硬编码颜色值。通过脚本或 `tailwind.config.js` 扩展，将 Ant Design 5 的 Design Token 桥接到 Tailwind CSS 变量（例如 `--color-primary`）。
    *   *价值*: 确保全局视觉一致性，避免“两种蓝色”的问题。
*   **协议仿真 (Schema Mocking)**: 
    *   *决策*: 在 `services/schema.ts` 中内置“本地回退”机制。开发环境下若后端接口未就绪，自动加载 `src/mock/schemas/*.json`。
    *   *价值*: 实现真正的“协议驱动并行开发”。
*   **局部容错 (Localized ErrorBoundary)**: 
    *   *决策*: 在核心渲染容器（如 `FormilyRenderer`）外层包裹 `ErrorBoundary`。
    *   *价值*: 防止由于 Schema 定义错误导致的页面级白屏崩溃。
